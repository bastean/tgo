...
func SetupScenario(sc *godog.ScenarioContext) {
	...
	sc.Then(`^I fill the (.+) list with (.+)$`, func(placeholder, value string) {
		element = page.Locator(fmt.Sprintf("#%s #%s", tab, strings.ToLower(placeholder)))

		assert.NoError(element.Click())

		element = page.GetByRole("textbox", playwright.PageGetByRoleOptions{Name: placeholder})

		assert.NoError(element.Fill(value))

		assert.NoError(element.Press("Enter"))
	})
