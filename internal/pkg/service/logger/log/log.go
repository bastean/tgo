...
import (
	...
	"github.com/bastean/tgo/pkg/context/infrastructure/record/log"
...
func Logo() {
	figureT := figure.NewFigure("t", "speed", true).Slicify()
	figureGo := figure.NewFigure("GO", "speed", true).Slicify()

	width := 0
	fixedWidth := 0

	for _, line := range figureT {
		width = len(line)
